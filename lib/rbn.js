...
class RbnClient extends EventEmitter {
  constructor() {
    ...
    this._filterCallsign = ''; // uppercase callsign to filter for
  ...
  connect({ host, port, callsign }) {
    ...
    this._filterCallsign = (callsign || '').toUpperCase();
    this._loggedIn = false;
  ...
  _parseSpotLine(line) {
    ...
    // Client-side filter: only emit spots for our callsign
    if (dxCall.toUpperCase() !== this._filterCallsign) return;
