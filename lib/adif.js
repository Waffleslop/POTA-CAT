...
function parseWorkedQsos(filePath) {
  ...
  for (const rec of records) {
    ...
    const entry = { date: f.QSO_DATE || '', ref: (f.SIG_INFO || '').toUpperCase() };
...
module.exports = { parseAdifFile, parseWorkedQsos, parseAllQsos, parseAllRawQsos, parseSqliteFile, parseSqliteConfirmed, isSqliteFile, normalizeMode, normalizeBand };
