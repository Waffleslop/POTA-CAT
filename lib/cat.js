...
class CatClient extends EventEmitter {
  ...
  tune(frequencyHz, mode, { split } = {}) {
    this._log(`tune() called: freq=${frequencyHz} mode=${mode} split=${!!split} connected=${this.connected}`);
    ...
    // Send mode command after a short delay — some radios (e.g. Yaesu FT-710)
    // reject multiple commands concatenated in a single write
    let delay = 100;
    if (mode) {
      const modeCode = mapMode(mode, frequencyHz);
      if (modeCode !== null) {
        setTimeout(() => {
          if (this.connected) this._write(`MD${modeCode};`);
        }, delay);
        delay += 100;
      }
    }
    // Enable or disable split mode (FT1 = split on, FT0 = split off)
    setTimeout(() => {
...
class RigctldClient extends EventEmitter {
  ...
  tune(frequencyHz, mode, { split } = {}) {
    ...
    // rigctld split: S 1 = split on, S 0 = split off
