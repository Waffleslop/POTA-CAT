...
All official POTACAT releases are signed using a certificate issued by the SignPath Foundation. The signing process verifies that binaries are built directly from the source code in this repository via an automated CI/CD pipeline.
...
POTACAT includes optional, opt-in telemetry that collects only a random anonymous ID, app version, OS, and session duration. No callsign, location, IP address, or identifying information is collected. Telemetry is off by default and can be disabled at any time in Settings.
